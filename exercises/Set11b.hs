...
hSelectLines h nums = let st = foldr Set.insert Set.empty nums
                          go n = do eof <- hIsEOF h
                                    if eof then return []
                                    else do line <- hGetLine h
                                            lines <- go (n+1)
                                            if Set.member n st then return (line:lines)
                                            else return lines
                      in go 1
...
interact' f state = do line <- getLine
                       let (continue, value, nst) = f (line, state)
                       putStrLn value
                       if continue then interact' f nst
                       else return nst
