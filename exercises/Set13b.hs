...
path maze place1 place2 = let (s, l) = runState (visit maze place1) []
                          in place2 `elem` l
...
instance Functor SL where
  -- implement fmap
  fmap f (SL op) = SL (\s -> let (x, s', msg) = op s in (f x, s', msg))
