...
--  +++ OK, passed 100 tests.
propCnt :: (Show a, Eq a) => [(a,Int)] -> a -> Property
propCnt output c = True === find
    where filteredOutput = filter (\(x, n) -> x == c) output
          find = if length filteredOutput == 0 then False
                                               else True

inputInOutput :: (Show a, Eq a) => [a] -> [(a,Int)] -> Property
inputInOutput input output = forAll (elements (nub input)) (propCnt output)
...
outputInInput input output = forAll (elements output) prop
                             where prop (c, freq) = length (filter (\x -> x == c) input) === freq
...
instance Arbitrary Arg where
  arbitrary = do
    t <- elements [0, 1]
    if t == 1 then do v <- elements [0..10]
                      return (Number v)
              else do c <- elements "abcxyz"
                      return (Variable c)
