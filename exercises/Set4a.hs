...
import Data.Array
import qualified Data.IntMap as Map.Map
...
longest (a:as) = foldr ck a as

ck a b
    | length a < length b = b
    | length a == length b && head b < head a = b
    | otherwise = a
...
incrementKey k [] = []
incrementKey k (v:vs)
    | fst v == k = (fst v, snd v+1) : incrementKey k vs
    | otherwise = v : incrementKey k vs
...
average xs = average' 0 0 xs
    where   average' s c [] = s / c
            average' s c (x:xs) = average' (s+x) (c+1) xs
...
freqs = foldr helper Map.empty
    where helper k mp = Map.insert k (Map.findWithDefault 0 k mp + 1) mp
...
transfer from to amount bank
    | not (Map.member from bank) || not (Map.member to bank) || amount <= 0 = bank
    | Map.findWithDefault 0 from bank < amount = bank
    | otherwise = Map.insert from (Map.findWithDefault 0 from bank - amount) added
        where added = Map.insert to (Map.findWithDefault 0 to bank + amount) bank
...
maxIndex as = fst (foldr cmp (head xs) xs)
    where xs = assocs as
          cmp a b 
            | snd a < snd b = b
            | otherwise = a
